...
########################################################################################################
        ...
        for child in tree.children:
            ...
            if (child.label == "EXPR"):
                ...
                print("UNKNOWN ASSIGNMENT STATEMENT: " + child.label)
        if (val != None):
             ...
             if(typeOfVar == "int"):
                 if (val.isdigit()):
                     ...
                     print(idStr)
                     k = 0
                     while k < len(symbolTable):
                         if( symbolTable[k][0] == val):
                             break;
                         k = k + 1
                     
                     if(k < len(symbolTable)):
                         assignPointerDirect(tree, symbolTable, output, idStr, val)
                     else:
                         assignPointerRet(tree, symbolTable, output, idStr, val)
                 else:
            ...
            if child.label == "EXPR_LIST":
		       ...
		       i = 0
                       ...
                       if(typeOfVar == "pointer"):
                           assignPointerRet(tree, symbolTable, output, returnVal, val)
    else:
        print("UNKNOWN")
        print(tree.children[0].label)
    ...
    for child in tree.children:
	    ...
	    notVar(tree, symbolTable, output, val, val)
            ...
            with open(output,"a") as outputFile:
                outputFile.write("#Parameter Assignment?")
		    ...
		    break;
                j = j + 1
            print(";lasdf;lkajsg;ljasd;lfjasdf;ljasdf;ljasdf;ljaksdf;lajsdf;lajsdf;ljasf")
	    print(tree.children[2])
            if(tree.children[2] is None):
                writeFuncCall(methName, output)
            print("COUNT IN FUNCEND " + str(count))
        #add something below for the end function varible to make isFunc falsea
        elif child.label == "AMP":
            print("FOUND AN AMP")
            passByValue = True;
        ############################################################
        elif (child.label == "FUNC_CHOICE"):
            print("FUNC IN PRIMARY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
            j = 0
...
def IDENT(tree, symbolTable, intLitTable, output, init):
    ...
    if symbolTable[i][4] != True:
        raise ParserError("Semantic Error: Variable not declared" + str(symbolTable[i]))
...
def assignPointerRet(tree, symbolTable, output, ident1, ident2):
    with open(output,"a") as outputFile:
        outputFile.write("\nlw $t0,"+ident2+"\nla $t1,"+ident1+"\nla $t2, 0($t0)\nsw $t2, 0($t1)\n")

def assignPointerDirect(tree, symbolTable, output, ident1, ident2):
    with open(output,"a") as outputFile:
        outputFile.write("\nlw $t0,"+ident2+"\nla $t1,"+ident1+"\nlw $t2, 0($t0)\nsw $t2, 0($t1)\n")
